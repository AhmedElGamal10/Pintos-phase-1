...
/* Called by the timer interrupt handler at each timer tick.
   ...
   void func(struct thread* t, void* x)
   {
        if (t == (struct thread*)x) {
            printf("+++++++++++++");
        }
        printf("thread : %s, %d, %d\n", t->name, (int)t->priority, (int)tick_count );
...
thread_tick (void)
{
  ...
  if (t == idle_thread)
    ...
    if (tick_count % 10 == 0) {
        //printf("+++++++++++++thread : %s, %d, %d\n", thread_current()->name, (int)thread_current()->priority, (int)round_to_int(mul_integer(thread_current()->recent_cpu, 1000)) );
       //thread_foreach(func, (void*)thread_current());
    }
  /*
...
//  list_sort(&ready_list, (list_less_func *) &thread_cmpr, NULL);
...
void thread_update_priority_recent_cpu(struct thread *t, void* dummy)
{
    ...
    //printf("before : %s : %d : %d : ld%d\n", t->name, (int)t->priority, (int)round_to_int(t->recent_cpu), (int)round_to_int(mul_integer(load_avg, 1000)));
    int32_t recent_cpu = t->recent_cpu;
    int32_t nice = (int32_t) t->nice;
//printf("rcpu = %d, load_avg = %d, nic = %d\n", (int)round_to_int(mul_integer(recent_cpu, 1000)), (int)round_to_int(mul_integer(load_avg, 1000)), t->nice);
    //calculate it once
...
//printf("     rcpu = %d\n\n", (int)round_to_int(mul_integer(recent_cpu, 1000)));
    thread_update_priority(t, dummy);
   // printf("after : %s : %d : %d : ld%d\n", t->name, (int)t->priority, (int)round_to_int(t->recent_cpu), (int)round_to_int(mul_integer(load_avg, 1000)));
}
...
void update_load_avg()
{
    ...
    int32_t for_debug = ready_threads;
    ...
    //printf("ready : %d, load_avg : %d, size : %d\n", (int)for_debug, (int)round_to_int(load_avg), size_priority_queue(mlf_p_queue));
}
...
//    ready_threads = get_as_fixed(ready_threads);
//    //divide by 60
//    ready_threads = div_integer(ready_threads, 60);
//
//    //temp = (59/60) * load_avg
//    int32_t temp_load_avg = load_avg;
//    temp_load_avg = mul_integer(div_integer(temp_load_avg, 60), 59);
//
//    temp_load_avg = add_fixed_point(temp_load_avg, ready_threads);
//    //update load_avg
//
